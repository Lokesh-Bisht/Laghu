...
import static dev.lokeshbisht.url_shortener_service.constants.ErrorMessages.URL_NOT_FOUND;
...
public class UrlMappingServiceImpl implements UrlMappingService {
    ...
    public ApiResponseDto<String> shortenUrl(String longUrl) {
        ...
        UrlMapping urlMapping = UrlMapping.builder()
            ...
            .createdAt(new Date())
            .build();
        return mapToApiResponseDto(hostUrl + "/" + urlMappingRepository.save(urlMapping).getShortUrl(), startTime);
    }
    ...
    public String getLongUrl(String shortUrl) {
        logger.info("Fetch long url using the short url: {}", shortUrl);
        Optional<UrlMapping> urlMapping = urlMappingRepository.findByShortUrl(shortUrl);
        if (urlMapping.isEmpty()) {
            logger.info("No url found for the given short url.");
